...
namespace Exercise3 {
  ...
  void Block_Tridiagonal::solve() {
    ...
    ///////// Using maps of the blocks to avoid unnecessary copies //////////
    using BlockMap = Eigen::Map<mat_type, 0, Eigen::OuterStride<> >;
    using VecMap   = Eigen::Map<vec_type>;
    ...
    BlockMap D0( D_bloks.data(), _n, _n, Eigen::OuterStride<>(D_bloks.outerStride()) ); // Map of the first diagonal block
    mat_type D_hat = D0;
    LU_type  lu( D_hat );
    ...
    */
    ...
    if ( _n_blk > 1 )
    {
      BlockMap U0( U_bloks.data(), _n, _n, Eigen::OuterStride<>(U_bloks.outerStride()) ); // Map of the first upper block
      U_hat[0] = lu.solve( U0 );
    } 
    //rhs_hat[0] = lu.solve( rhs_block(0) );
    VecMap b0( B_bloks.data(), _n ); // b0 is a view of the first block of the right-hand side
    ...
    for ( integer blk = 1; blk < _n_blk; ++blk ) 
    {
      ...
      ///////// Using maps of the blocks to avoid unnecessary copies //////////
      //////////////////////////////////////////////////////////////////////////
      BlockMap Dblk( D_bloks.data() + blk * _n * D_bloks.outerStride(), _n, _n, Eigen::OuterStride<>(D_bloks.outerStride()) );
      BlockMap Lprev( L_bloks.data() + (blk-1) * _n * L_bloks.outerStride(), _n, _n, Eigen::OuterStride<>(L_bloks.outerStride()) );
      VecMap bblk( B_bloks.data() + blk*_n, _n );
      ...
      */
      ...
      if ( blk < _n_blk-1 ) 
      {
        BlockMap Ublk( U_bloks.data() + blk * _n * U_bloks.outerStride(), _n, _n, Eigen::OuterStride<>(U_bloks.outerStride()) );
    ...
    //   x_{m-1} = r̂_{m-1},
    //   x_i     = r̂_i - Û_i x_{i+1}.
    VecMap x_last( X_bloks.data() + (_n_blk-1)*_n, _n );
    x_last = rhs_hat[_n_blk-1];

    for ( integer blk = _n_blk-2; blk >= 0; --blk ) 
    {
      VecMap x_next( X_bloks.data() + (blk+1)*_n, _n );
      VecMap x_curr( X_bloks.data() + blk*_n, _n );
